...
| `login-focus-ring.png` *(bonus)* | 1280×800 | `/login` — button focused via keyboard | 1280×800 |
...
*None — toggle is functional, visible, accessible, no overflow, no layout shift, no border overlap, no regression at 320–390 widths.*
...
*Computed-style dumps, overflow checks, and Tab-focus logs are in `raw-results.json` / `focus-test.json` next to the PNGs.*
