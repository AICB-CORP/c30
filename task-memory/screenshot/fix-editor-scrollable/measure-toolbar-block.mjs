...
function editorHtmlToolbarBlock({ many, carousel, count = 10 }) {
  ...
  // Mirrors CURRENT RetroEditor.tsx after change: toolbar wrap block not scrollable
  // outer max-h-[90vh] flex-col overflow-hidden
  // middle flex min-h-0 flex-1 flex-col overflow-hidden  (not overflow-y-auto)
  // toolbar outer flex-shrink-0, inner flex flex-wrap (no overflow-x-auto)
  // editor-area flex min-h-[180px] flex-col overflow-hidden, inner flex-1 overflow-y-auto
  return `
  <div class="fixed inset-0 z-50 flex items-center justify-center overflow-hidden bg-black/80 p-2" data-testid="modal-outer">
    <div class="retro-box flex w-[95vw] max-w-3xl max-h-[90vh] flex-col overflow-hidden" data-testid="retro-box">
      ...
      <!-- Middle — toolbar block (not scrollable) + media + editor scrollable -->
      <div class="flex min-h-0 flex-1 flex-col gap-2 overflow-hidden pr-1" data-testid="middle">
        <div class="flex-shrink-0 rounded-lg border-2 border-[#ff69b4]/60 bg-black/40 p-2" data-testid="toolbar-outer">
...
async function testViewport(viewport, name, many, carousel) {
  ...
  const html = editorHtmlToolbarBlock({ many, carousel, count: 10 });
  ...
  const metrics = await page.evaluate(() => {
    ...
    const toolbarOverflowsHorizontally = toolbar.scrollWidth > toolbar.clientWidth + 1;
    const middleOverflows = middle.scrollHeight > middle.clientHeight + 1;
    // count toolbar rows by grouping buttons by top position
    const toolbarChildren = Array.from(toolbar.children);
    let rows = 1;
    if (toolbarChildren.length > 0) {
      const tops = toolbarChildren.map((el) => Math.round(el.getBoundingClientRect().top));
      const uniqueTops = [...new Set(tops)];
      rows = uniqueTops.length;
    }
    // also compute expected: is flex-wrap wrap and not scrollable?
    return {
      viewport: { w: vw, h: vh },
      docScrollWidth,
  ...
  // Test toolbar fixed when editor scrolled: record toolbar Y before and after editor scroll
  if (many) {
    ...
    const afterEditorScroll = await page.evaluate(() => {
      const toolbar = document.querySelector('[data-testid="toolbar"]');
      const tr = toolbar.getBoundingClientRect();
      const publish = document.querySelector('[data-testid="publish-btn"]');
      const pr = publish.getBoundingClientRect();
      ...
      const middle = document.querySelector('[data-testid="middle"]');
      return {
        toolbar: { y: tr.top, bottom: tr.bottom, h: tr.height },
        publish: {
          y: pr.top,
          bottom: pr.bottom,
          h: pr.height,
    ...
    console.log("After editor scroll to bottom:", JSON.stringify(afterEditorScroll, null, 2));
    // check that toolbar Y unchanged (fixed)
    const toolbarFixed = Math.abs(afterEditorScroll.toolbar.y - before.toolbar.y) < 2;
    console.log(
      `Toolbar fixed after editor scroll? ${toolbarFixed} (before y ${before.toolbar.y} -> after ${afterEditorScroll.toolbar.y})`,
    );
    ...
    // also capture a second screenshot after scrolling to top to verify toolbar still visible
    await page.evaluate(() => {
      const s = document.querySelector('[data-testid="editor-scroll"]');
      if (s) s.scrollTop = 0;
    });
    ...
    // also test scrolling editor even when empty (should not scroll much)
    const midCheck = await page.evaluate(() => {
      ...
      return {
        ...
        editor: {
          ...
          hasScroll: s.scrollHeight > s.clientHeight + 1,
          overflowY: getComputedStyle(s).overflowY,
        },
      };
    });
    console.log("Scroll check empty:", midCheck);
  }
  await browser.close();
  return metrics;
}
...
const results = {};
results["toolbar-block-desktop-empty"] = await testViewport(
  { width: 1280, height: 800 },
  "toolbar-block-desktop",
  false,
  false,
);
results["toolbar-block-mobile-375-empty"] = await testViewport(
  { width: 375, height: 812 },
  "toolbar-block-mobile-375",
  false,
  false,
);
results["toolbar-block-mobile-360-empty"] = await testViewport(
  { width: 360, height: 800 },
  "toolbar-block-mobile-360",
  false,
  false,
);
results["editor-scrollable-desktop-many"] = await testViewport(
  { width: 1280, height: 800 },
  "editor-scrollable-desktop",
  true,
  true,
);
results["editor-scrollable-mobile-375-many"] = await testViewport(
  { width: 375, height: 812 },
  "editor-scrollable-mobile-375",
  true,
  true,
);
results["editor-scrollable-mobile-360-many"] = await testViewport(
  { width: 360, height: 800 },
  "editor-scrollable-mobile-360",
  true,
  true,
);
results["editor-scrollable-stacked-desktop"] = await testViewport(
  { width: 1280, height: 800 },
  "editor-scrollable-stacked-desktop",
  true,
  false,
);
results["editor-scrollable-stacked-mobile-375"] = await testViewport(
  { width: 375, height: 812 },
  "editor-scrollable-stacked-mobile-375",
  true,
  false,
);
...
const metricsOut = {
  ...
  description:
    "Re-test after toolbar changed to flex-wrap block (not scrollable), editor flex-1 overflow-y-auto, publish fixed",
  results,
};
fs.writeFileSync(
  path.join(screenshotDir, "metrics-toolbar-block.json"),
  JSON.stringify(metricsOut, null, 2),
);
fs.writeFileSync(path.join(screenshotDir, "metrics.json"), JSON.stringify(metricsOut, null, 2));
console.log("\nMetrics written to metrics-toolbar-block.json and metrics.json");
